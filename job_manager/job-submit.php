...
<form action="<?php echo esc_url( $action ); ?>" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">
	...
	<?php if ( job_manager_user_can_post_job() || job_manager_user_can_edit_job( $job_id ) ) : ?>
		...
		foreach ( $job_fields as $key => $field ) :
			...
			if($field['priority'] > 99 && $after99 == false) {
				echo '<h2>Contact Information for Person Submitting Job Posting</h2>';
				echo '<p>This information will only be used if there is an issue with job posting submission.</p>';
				$after99 = true;
			}
			?>
			<fieldset class="fieldset-<?php echo esc_attr( $key ); ?> fieldset-type-<?php echo esc_attr( $field['type'] ); ?>">
				<label <?php if($field['required']) echo 'class="label-required" '; ?>for="<?php echo esc_attr( $key ); ?>"><?php echo wp_kses_post( $field['label'] ) . wp_kses_post( apply_filters( 'submit_job_form_required_label', $field['required'] ? '' : ' <small>' . __( '(optional)', 'wp-job-manager' ) . '</small>', $field ) ); ?></label>
